...
class NewTinyRecursiveModel(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 384):
        ...
        self.head_side = nn.Linear(self.y_dim, 3)
        self.head_qty = nn.Linear(self.y_dim, 1)
        self.head_px = nn.Linear(self.y_dim, 1)
        self.head_tp = nn.Linear(self.y_dim, 1)
        self.head_sl = nn.Linear(self.y_dim, 1)
        self.head_tif = nn.Linear(self.y_dim, 3)
        self.head_lev = nn.Linear(self.y_dim, 1)
        self.head_score = nn.Linear(self.y_dim, 1)
        self.head_ordertype = nn.Linear(self.y_dim, 2)
        self.head_reduce = nn.Linear(self.y_dim, 1)
